...
def validate(val_model, val_loader, num_classes, device, saver=None, multi_frame=False):
	...
	with torch.set_grad_enabled(False):
		...
		for _iter, (image, frame) in tqdm(enumerate(val_loader), total=len(val_loader), dynamic_ncols=True):
			...
			if multi_frame:
				results = val_model(inputs, frame)
				if results is not None and saver is not None:
					out_pred, out_frame = results
					saver.save_frame(out_frame, out_pred)
			else:
				pred = val_model(inputs)
				if saver is not None:
					saver.save_frame(frame, pred)
	...
	for video_info in video_cfg["videos-info"]:
		...
		for video_exper in video_cfg["expers"]:
			...
			validate(model_val, 
				val_dataloader, 
				num_classes, 
				device, saver=results_saver, multi_frame=("multi" in video_exper["name"]))
