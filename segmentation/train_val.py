...
from segmentation.val import validate
...
if __name__ == "__main__":
	...
	args = parse_args()
	exper_name = os.path.join(Path(args.config).parts[0], Path(args.config).parts[-1].split(".")[0])
	dataset_dir = os.path.join('list', args.dataset)

	config = utils.get_cfgs_v2(args.config)
