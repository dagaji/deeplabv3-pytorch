...
from deeplabv3.model.deeplab import init_conv
...
if __name__ == "__main__":
	...
	for glob in Path(os.path.join(dataset_dir)).glob("*"):
		...
		model_train = get_model(4, training_cfg["model"], False).to(device)
		...
		if last_checkpoint_path is not None:
			...
			model_train.classifier = nn.Conv2d(256, 3, 1, 1, 0, 1, bias=False)
			init_conv(model_train.classifier)
			model_train.classifier = model_train.classifier.to(device)
		else:
			current_epoch = 0
