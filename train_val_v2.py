...
def get_dataloader(id_list_path, dataset_cfg, batch_size):
...
if __name__ == "__main__":
	...
	for glob in Path(os.path.join(dataset_dir)).glob("*"):
		...
		id_list_path = os.path.join(partition, '{}.txt').format('train')
		train_loader = get_dataloader(id_list_path, training_cfg['dataset'], training_cfg['batch_size'])
		...
		train(models['train'], train_loader, criterion, optimizer, scheduler, device, training_cfg)
		train_model(models, dataloaders, criterion, optimizer, scheduler, device, training_cfg, val_cfg, num_classes, num_epochs=args.num_epochs)